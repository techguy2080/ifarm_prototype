...
import { Search, DollarSign, Globe, Calendar } from 'lucide-react';
import DashboardContainer from '@/components/DashboardContainer';
import { getCurrentUser } from '@/lib/auth';
import { mockAnimalSales, mockTenants } from '@/lib/mockData';
...
export default function AdminSalesPage() {
  ...
  let filteredSales = mockAnimalSales;

  if (selectedTenant !== 'all') {
    filteredSales = filteredSales.filter(s => s.tenant_id === selectedTenant);
  }

  if (searchTerm) {
    filteredSales = filteredSales.filter(s =>
      s.animal_tag?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
              ...
              <Box sx={{ flex: 1 }}>
                <Typography variant="h3" component="h1" sx={{ color: 'white', mb: 0.5, fontWeight: 400 }}>
                  All Sales (System-Wide)
                </Typography>
                <Typography variant="h6" sx={{ color: 'rgba(255, 255, 255, 0.95)', fontWeight: 400 }}>
                  View sales transactions across all tenants
                </Typography>
              </Box>
              <Box sx={{ textAlign: 'right', color: 'white' }}>
                <Typography variant="h4" fontWeight="700">UGX {totalRevenue.toLocaleString()}</Typography>
        ...
        <Card sx={{ mb: 3, borderRadius: 3, boxShadow: '0 4px 20px rgba(5, 150, 105, 0.15)' }}>
          <CardContent>
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 2 }}>
              <TextField
                fullWidth
                placeholder="Search by animal tag or buyer..."
                value={searchTerm}
              ...
              <Table>
                <TableHead>
                  <TableRow sx={{ background: alpha('#16a34a', 0.05) }}>
                    ...
                    <TableCell sx={{ fontWeight: 700 }}>Animal Tag</TableCell>
                ...
                <TableBody>
                  {filteredSales.map((sale) => {
                    const tenant = mockTenants.find(t => t.tenant_id === sale.tenant_id);
                    return (
                      <TableRow key={sale.sale_id} hover>
                        ...
                        <TableCell>{sale.animal_tag || 'N/A'}</TableCell>
                        ...
                        <TableCell>
                          <Chip
                            label={sale.payment_method?.replace('_', ' ') || 'N/A'}
                            size="small"
                            variant="outlined"
