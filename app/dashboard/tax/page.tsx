...
import { Receipt, Percent, Calculator, FileCheck, TrendingUp, DollarSign, Calendar, Filter } from 'lucide-react';
import DashboardContainer from '@/components/DashboardContainer';
import { getCurrentUser, hasPermission } from '@/lib/auth';
import { mockTaxCalculations, mockTaxRecords, mockTaxRates, mockTaxConfiguration, mockFarms } from '@/lib/mockData';
...
export default function TaxManagementPage() {
  ...
  const taxSummary = useMemo(() => {
    const calculations = filterFarm === 'all' 
      ? mockTaxCalculations 
      : mockTaxCalculations.filter(tc => tc.farm_id === filterFarm);
  ...
  const filteredRecords = useMemo(() => {
    return filterFarm === 'all'
      ? mockTaxRecords
      : mockTaxRecords.filter(tr => tr.farm_id === filterFarm);
  }, [filterFarm]);
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flex: 1 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body2" sx={{ color: 'rgba(255, 255, 255, 0.9)' }}>
                    Uganda Revenue Authority (URA) Compliant Tax Tracking
                  </Typography>
                </Box>
              </Box>
          ...
          <CardContent sx={{ p: 3 }}>
            <Stack direction="row" spacing={2} alignItems="center">
              ...
              <Chip
                ...
                color={filterFarm === 'all' ? 'primary' : 'default'}
                sx={{ cursor: 'pointer' }}
              />
              {mockFarms.map(farm => (
                <Chip
                  ...
                  onClick={() => setFilterFarm(farm.farm_id)}
                  color={filterFarm === farm.farm_id ? 'primary' : 'default'}
                  sx={{ cursor: 'pointer' }}
                />
              ))}
            </Stack>
                    ...
                    {filteredCalculations.length === 0 ? (
                      ...
                      filteredCalculations.map((calc) => (
                        <TableRow key={calc.tax_calculation_id} hover>
                          ...
                          <TableCell>
                            {mockFarms.find(f => f.farm_id === calc.farm_id)?.farm_name || 'N/A'}
                          </TableCell>
                        </TableRow>
                      ))
                    )}
          ...
          <CardContent sx={{ p: 3 }}>
            <Stack direction="row" spacing={2} alignItems="center">
              ...
              <Box sx={{ flex: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Tax calculations are based on Uganda's tax regulations. VAT rate: 18%, Fiscal Year: July 1 - June 30.
                  {mockTaxConfiguration.ura_vat_registered ? (
                    <span> Your business is VAT registered with URA (VAT Number: {mockTaxConfiguration.ura_vat_number}).</span>
                  ) : (
                    <span> VAT registration threshold: UGX {mockTaxConfiguration.vat_registration_threshold.toLocaleString()}.</span>
                  )}
                </Typography>
              </Box>
            </Stack>
